...
const sumarCosas = (dbIds,parametro) => {
    viewer.model.getBulkProperties(dbIds, [parametro], res => {
        ...
        res.forEach(item => {
            var v = item.properties[0].displayValue
            if (!isNaN(v)) {
                total += v
            }
        })
...
// const sumarVolumen = () => {
//     viewer.model.getBulkProperties(viewer.getSelection(), ['Volume'], res => {
//         var volumen = 0.00
//         res.forEach(item => {
...
// const sumarArea = () => {
//     viewer.model.getBulkProperties(viewer.getSelection(), ['Area'], res => {
//         var area = 0.00
//         res.forEach(item => {
